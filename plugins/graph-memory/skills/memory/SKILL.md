...
- Use `memory_latest` for when something last happened, `memory_evidence` for sources, and `memory_render` to visualize connections. Evidence accepts 1–10 fact IDs in one call: `detail:"index"` lists short claims and source IDs, compact returns bounded excerpts, and `detail:"full"` returns exact quotes and source context. Expand only relevant IDs; use full evidence before sourced writes or when an excerpt leaves a verification gap.
...
- A conflict label can reflect an erroneous exclusive slot. When the original evidence or user establishes compatible alternatives, use `memory_allow_alternatives` to clear that grouping while preserving both facts. Do not retract a valid alternative merely to hide the conflict.
...
Entity search requires a nonempty name; it cannot list everyone with an empty query. If a known entity's question returns no matches, try fewer distinctive terms or omit the question for bounded entity context; do not treat that fallback as an answer to the original question. Historical calls share a memory budget: on a busy response, retry sequentially after a short delay with the same cutoff. Do not silently replace a required historical cutoff with a live query.
...
Before writing a manual note, check whether recall already contains the original statement. Direct `memory_ingest` writes need a `sources` mapping from submitted message IDs to `source_message_id` values returned by full `memory_evidence`, using exact source excerpts. Do not turn your paraphrase into a fabricated user message or timestamp.

Interpret write receipts narrowly. `context_only_message_ids` identifies submitted messages that cannot produce searchable facts from this manual write; other sourced messages may still be processed. `available_for_recall:false` does not establish that the information is absent elsewhere or permanently lost. Say “Context accepted; searchable recall not confirmed.” Background ingestion is a separate path that may later capture the original conversation; neither promise that it will nor say the information cannot carry over. Check targeted recall before claiming a searchable fact was saved. Do not repeat unsourced writes or recommend editing instruction files solely because of this receipt; discuss a standing rule only when that is the user's goal.
