...
Before writing a manual note, check whether recall already contains the original statement. Direct `memory_ingest` writes need a `sources` mapping from submitted message IDs to `source_message_id` values returned by `memory_evidence`, using exact source excerpts. Do not turn your paraphrase into a fabricated user message or timestamp. If the receipt lists `context_only_message_ids`, those messages cannot produce searchable facts; say context was accepted, not that a recallable fact was saved. Background ingestion may provide the original conversation later, but verify recall before claiming it did.
